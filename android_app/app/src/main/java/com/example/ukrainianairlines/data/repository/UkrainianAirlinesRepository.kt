...
class UkrainianAirlinesRepository(private val tokenProvider: (() -> String?)? = null) {
    ...
    fun getOrders(): Flow<Result<List<Order>>> = flow {
        try {
            ...
            if (response.isSuccessful) {
                response.body()?.let { paginatedOrders ->
                    emit(Result.success(paginatedOrders.results))
                } ?: emit(Result.failure(Exception("Empty response")))
            } else {
    ...
    suspend fun getUserOrders(page: Int = 1, limit: Int = 10): Result<PaginatedOrders> {
        return try {
            val response = getApi().getOrders()
            if (response.isSuccessful) {
                ...
                Result.failure(Exception("Failed to get user orders: ${response.message()}"))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
